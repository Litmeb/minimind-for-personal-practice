...
class MiniMindBlock(nn.Module):
    def __init__(self, layer_id: int, config: MiniMindConfig, adapter1: nn.Module = None, adapter2: nn.Module = None):
        super().__init__()
    ...
    def forward(self, hidden_states, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        if self.adapter2 is None:
            hidden_states = hidden_states + self.mlp(self.post_attention_layernorm(hidden_states))
        else:
            hidden_states = hidden_states + self.adapter2(self.mlp(self.post_attention_layernorm(hidden_states)))
...
class MiniMindModel(nn.Module):
    ...
    def forward(self,
        ...
        if input_ids is not None:
            ...
            # 从 inputs_embeds 推断形状
            batch_size, seq_length, _ = inputs_embeds.shape
            hidden_states = self.dropout(inputs_embeds)
        else:
            # 如果两者都为 None，尝试从 past_key_values 推断
            if past_key_values is not None and len(past_key_values) > 0 and past_key_values[0] is not None:
                batch_size = past_key_values[0][0].shape[0]
                seq_length = 1  # 后续步骤通常只处理单个token
                raise ValueError("input_ids and inputs_embeds cannot both be None")
            else:
                raise ValueError("input_ids and inputs_embeds cannot both be None when past_key_values is also None")
